...
from search_agent import search_agent,fetch_serp_results
...
async def run(query: str, answers: str, state: list[str]):
    ...
    for item in searches:
        # Step 1: Fetch raw snippets from SerpAPI
        snippets = fetch_serp_results(item.query)
        
        # Step 2: Summarize those snippets with your search_agent
        summarize_input = f"Search query: {item.query}\n\nResults:\n{snippets}"
        search_res = await Runner.run(search_agent, summarize_input)
